...
export function getContractTemplates(): ContractTemplate[] {
  return [
    ...
    },
    {
      ...
      description: "Contrato de prestação de serviços com escopo, prazos e condições",
      icon: "📋",
      pages: [
        {
          id: tplId(), backgroundOpacity: 0.5, elements: [
            makeRect(0, 0, 794, 60, { fill: "#1e40af", stroke: "transparent" }),
            makeText(40, 15, 600, 28, "CONTRATO DE PRESTAÇÃO DE SERVIÇOS", { fontSize: 18, fontWeight: "bold", color: "#ffffff" }),
            makeText(40, 42, 300, 15, "Ref: {{numero_contrato}}", { fontSize: 10, color: "#93c5fd" }),

            makeText(40, 80, 714, 20, "CONTRATANTE", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 98, 714, { stroke: "#1e40af" }),
            makeText(40, 105, 714, 20, "Nome/Razão Social: {{cliente_nome}}"),
            makeText(40, 128, 350, 20, "CPF/CNPJ: {{cliente_cpf}}"),
            makeText(400, 128, 354, 20, "Tel: {{cliente_telefone}}"),
            makeText(40, 151, 714, 20, "Endereço: {{cliente_endereco}}"),
            makeText(40, 174, 350, 20, "E-mail: {{cliente_email}}"),

            makeText(40, 210, 714, 20, "CONTRATADA", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 228, 714, { stroke: "#1e40af" }),
            makeText(40, 235, 714, 20, "Razão Social: {{empresa_nome}}"),
            makeText(40, 258, 350, 20, "CNPJ: {{empresa_cnpj}}"),
            makeText(400, 258, 354, 20, "Tel: {{empresa_telefone}}"),
            makeText(40, 281, 714, 20, "Endereço: {{empresa_endereco}}"),

            makeText(40, 320, 714, 20, "OBJETO DO CONTRATO", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 338, 714, { stroke: "#1e40af" }),
            makeText(40, 345, 714, 50, "O presente contrato tem por objeto a prestação dos seguintes serviços pela CONTRATADA ao CONTRATANTE:", { fontSize: 11, color: "#374151" }),

            makeTable(40, 400, 714, 130, [
              ["Serviço", "Descrição", "Prazo"],
              ["{{servico_1}}", "{{desc_servico_1}}", "{{prazo_servico_1}}"],
              ["{{servico_2}}", "{{desc_servico_2}}", "{{prazo_servico_2}}"],
              ["{{servico_3}}", "{{desc_servico_3}}", "{{prazo_servico_3}}"],
            ]),

            makeText(40, 550, 714, 20, "CONDIÇÕES FINANCEIRAS", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 568, 714, { stroke: "#1e40af" }),
            makeTable(40, 575, 714, 100, [
              ["Item", "Valor"],
              ["Valor Total dos Serviços", "{{valor_total}}"],
              ["Forma de Pagamento", "{{forma_pagamento}}"],
              ["Condições", "{{condicoes_pagamento}}"],
            ]),

            makeText(40, 695, 714, 20, "VIGÊNCIA E PRAZOS", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 713, 714, { stroke: "#1e40af" }),
            makeText(40, 720, 714, 40, "O presente contrato terá vigência de {{prazo_meses}} meses, com início em {{data_inicio}} e término previsto em {{data_fim}}, podendo ser prorrogado mediante acordo entre as partes.", { fontSize: 11, color: "#374151" }),

            makeText(40, 780, 714, 20, "CLÁUSULAS GERAIS", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 798, 714, { stroke: "#1e40af" }),
            makeText(40, 805, 714, 160, "1. A CONTRATADA se compromete a executar os serviços com qualidade e dentro dos prazos acordados.\n\n2. Qualquer alteração no escopo dos serviços deverá ser formalizada por aditivo contratual.\n\n3. A rescisão antecipada por qualquer das partes deverá ser comunicada com {{prazo_aviso}} dias de antecedência.\n\n4. Multa por descumprimento: {{multa_rescisao}}% do valor total do contrato.\n\n5. Foro eleito: Comarca de {{cidade_foro}}.", { fontSize: 11, color: "#374151" }),
          ],
        },
        {
          id: tplId(), backgroundOpacity: 0.5, elements: [
            makeRect(0, 0, 794, 35, { fill: "#1e40af", stroke: "transparent" }),
            makeText(40, 8, 714, 20, "CONTRATO DE PRESTAÇÃO DE SERVIÇOS — Página 2", { fontSize: 11, fontWeight: "bold", color: "#ffffff" }),

            makeText(40, 55, 714, 20, "OBRIGAÇÕES DA CONTRATADA", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 73, 714, { stroke: "#1e40af" }),
            makeText(40, 80, 714, 120, "a) Executar os serviços contratados com profissionalismo e qualidade;\n\nb) Cumprir os prazos estipulados neste contrato;\n\nc) Manter sigilo sobre informações confidenciais do CONTRATANTE;\n\nd) Comunicar imediatamente qualquer impedimento na execução dos serviços;\n\ne) Fornecer relatórios de progresso quando solicitado.", { fontSize: 11, color: "#374151" }),

            makeText(40, 220, 714, 20, "OBRIGAÇÕES DO CONTRATANTE", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 238, 714, { stroke: "#1e40af" }),
            makeText(40, 245, 714, 100, "a) Efetuar os pagamentos nas datas acordadas;\n\nb) Fornecer as informações necessárias para execução dos serviços;\n\nc) Designar um responsável para acompanhamento dos trabalhos;\n\nd) Comunicar eventuais alterações no escopo com antecedência.", { fontSize: 11, color: "#374151" }),

            makeText(40, 370, 714, 20, "DISPOSIÇÕES FINAIS", { fontSize: 12, fontWeight: "bold", color: "#1e40af" }),
            makeLine(40, 388, 714, { stroke: "#1e40af" }),
            makeText(40, 395, 714, 60, "As partes elegem o foro da Comarca de {{cidade_foro}} para dirimir quaisquer dúvidas ou controvérsias oriundas do presente contrato, renunciando a qualquer outro, por mais privilegiado que seja.", { fontSize: 11, color: "#374151" }),

            makeText(40, 480, 714, 18, "{{cidade_foro}}, {{data_contrato}}", { fontSize: 12, color: "#374151", textAlign: "center" }),

            makeText(40, 540, 340, 20, "______________________________", { textAlign: "center" }),
            makeText(40, 565, 340, 15, "CONTRATADA", { textAlign: "center", fontSize: 10, color: "#6b7280" }),
            makeText(40, 580, 340, 15, "{{empresa_nome}}", { textAlign: "center", fontSize: 9, color: "#9ca3af" }),

            makeText(414, 540, 340, 20, "______________________________", { textAlign: "center" }),
            makeText(414, 565, 340, 15, "CONTRATANTE", { textAlign: "center", fontSize: 10, color: "#6b7280" }),
            makeText(414, 580, 340, 15, "{{cliente_nome}}", { textAlign: "center", fontSize: 9, color: "#9ca3af" }),

            makeText(200, 650, 394, 20, "______________________________", { textAlign: "center" }),
            makeText(200, 675, 394, 15, "TESTEMUNHA", { textAlign: "center", fontSize: 10, color: "#6b7280" }),
          ],
        },
      ],
    },
