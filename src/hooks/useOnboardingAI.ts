      ...
      if (step === "horario") {
        ...
        if (/sem\s+hor[aá]rio|nenhum|pular|n[ãa]o/i.test(lower)) {
          ...
          if (timeMatch) {
            ...
            appendAssistant("❌ Não entendi o horário. Use o formato **HH:MM** (ex: 14:00) ou diga **sem horário**.");
            return true;
          }
        }
        setTaskWizard(prev => ({ ...prev, horario, step: "descricao" }));
        appendAssistant(`✅ Horário: **${horario || "Sem horário"}**\n\n📝 **Qual a descrição da tarefa?**\n\n_Descreva os detalhes ou digite "pular" para não adicionar descrição._`);
        return true;
      }
      ...
      if (step === "notificacao") {
        ...
        if (/sem\s+notifica[çc][ãa]o|nenhum|pular|n[ãa]o/i.test(lower)) {
          ...
          if (numMatch) {
            ...
            appendAssistant("❌ Não entendi. Informe um número de minutos (ex: **15**) ou diga **sem notificação**.");
            return true;
          }
        }
        ...
        if (error) {
          console.error("[Mia] Task wizard insert error:", JSON.stringify(error));
          appendAssistant(`❌ Não consegui criar a tarefa: ${error.message || "Erro desconhecido"}`);
        } else {
          // Set notification preference if specified
